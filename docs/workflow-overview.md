...
1. ログ出力・アサーション・デバッグプリントをコードに追加する
2. ユーザーに再実行と出力結果の共有を依頼する
3. 受け取った出力を分析して根本原因を特定する
4. 根本原因が判明したら「再計画」または「微修正」を選択して対応する
5. 修正完了前に追加したデバッグコードを必ず削除してから動作確認を依頼する
